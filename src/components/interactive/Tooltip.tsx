...
const Tooltip: React.FC<TooltipProps> = ({ content, children, position = "top", className }) => {
	...
	return (
		<div
			className={cn("c-tooltip relative inline-block", className)}
		...
		>
			...
			{isVisible && (
				<div
					className={cn(
						"absolute z-50 px-3 py-2 text-sm text-white bg-gray-900 rounded whitespace-nowrap pointer-events-none",
						positionClasses[position]
					)}
					role="tooltip"
				>
					{content}
					<div className={cn("absolute w-0 h-0 border-4 border-solid", arrowClasses[position])} />
				</div>
			)}
		</div>
	);
